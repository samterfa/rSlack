...
#' @param text A \code{\link{text.object}} that defines the button's text. Can only be of type: plain_text. Maximum length for the text in this field is 75 characters.
...
#' @param confirm A \code{\link{confirm.object}} that defines an optional confirmation dialog after the button is clicked.
#' @return A button element
#' @seealso \url{https://api.slack.com/reference/block-kit/block-elements#button}
#' @export
button.element <- function(type = 'button', text, action_id = NULL, url = NULL, value = NULL, style = NULL, confirm = NULL){
  
  assertthat::assert_that(type == 'button', 
                          all('slack.text.object' %in% class(text)))
  
  obj <- as.list(environment())
  class(obj) <- append(class(obj), 'slack.button.element')
